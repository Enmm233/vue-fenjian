...
axios.interceptors.request.use(
  config => {
    ...
    if (store.state.token) {
      config.headers.Authorization = JSON.parse(localStorage.getItem('sort_token')).sort_token
       // config.headers.Authorization =localStorage.getItem('sort_token');
...
axios.interceptors.response.use(
  ...
  if(response.data.code==401){
    ...
    {
      var data = new Object();
      data.sort_token=response.headers.authorization;
